#include <cs50.h>
#include <stdio.h>

// Select every alternate number, starting from second-last digit
// Multiply each of these numbers by 2
// Add these digits together
// Add this to the sum of digits that were not multiplied by 2
// If the last digit in this sum is 0, the card is valid
// If valid, identify the type of credit card used.

int main(void)
{
    long card;
    do
    {
        card = get_long("Card no: ");
    } while (card < 0); // card must be greater then 0

    int card1, card2, card3, card4, card5, card6, card7, card8;
    // starting from the second-last digit
    card1 = ((card % 100) / 10) * 2;
    card2 = ((card % 10000) / 1000) * 2;
    card3 = ((card % 1000000) / 100000) * 2;
    card4 = ((card % 100000000) / 10000000) * 2;
    card5 = ((card % 10000000000) / 1000000000) * 2;
    card6 = ((card % 1000000000000) / 100000000000) * 2;
    card7 = ((card % 100000000000000) / 10000000000000) * 2;
    card8 = ((card % 10000000000000000) / 10000000000000) * 2;

    // split the first digit and the second digit
    card1 = (card1 % 100 / 10) + (card1 % 10);
    card2 = (card2 % 100 / 10) + (card2 % 10);
    card3 = (card3 % 100 / 10) + (card3 % 10);
    card4 = (card4 % 100 / 10) + (card4 % 10);
    card5 = (card5 % 100 / 10) + (card5 % 10);
    card6 = (card6 % 100 / 10) + (card6 % 10);
    card7 = (card7 % 100 / 10) + (card7 % 10);
    card8 = (card8 % 100 / 10) + (card8 % 10);

    // sum up all these digits
    int sum1 = card1 + card2 + card3 + card4 + card5 + card6 + card7 + card8;

    // find digits that were not multiplied
    int card9 + card10 + card11 + card12 + card13 + card14 + card15 + card16;
    card9 = (card % 10);
    card10 = ((card % 1000) / 100);
    card11 = ((card % 100000) / 10000);
    card12 = ((card % 10000000) / 1000000);
    card13 = ((card % 1000000000) / 100000000);
    card14 = ((card % 100000000000) / 10000000000);
    card15 = ((card % 10000000000000) / 1000000000000);
    card15 = ((card % 1000000000000000) / 100000000000000);

    int sum2 = card9 + card10 + card11 + card12 + card13 + card14 + card15 + card16;
    int sum3 = sum1 + sum2;

    int length = 0;
    long visa = card;
    long amex = card;
    long master = card;

    if ((sum3 % 10) != 0)
    {
        printf("%s\n", "INVALID");
        return 0;
    }

    // If valid, identify the type of credit card used.
    // Differentiate between VISA, Master and Amex
    while (card > 0)
    {
        card = card / 10; // keep dividing the card by ten, to get the length of the card
        length++;
    }

    // Identify if the card is a VISA
    while (visa >= 10)
    {
        visa /= 10;
    }
    if (visa == 4 && (length == 13 || length == 16))
    {
        printf("%s\n", "VISA");
        return 0;
    }

    // Identify if the card is an AMEX
    while (amex >= 10000000000000)
    {
        amex /= 10000000000000;
    }
    if (length == 15 && (amex == 34 || amex == 37))
    {
        printf("%s\n", "AMEX");
        return 0;
    }

    // Identify if the card is a Mastercard
    while (master >= 100000000000000)
    {
        master /= 100000000000000;
    }
    if (length == 16 && (master == 51 || master == 52 || master == 54 || master == 55))
    {
        print("%s\n", "MASTERCARD");
        return 0;
    }
    else
        print("%s\n", "INVALID");
    return 0;
}